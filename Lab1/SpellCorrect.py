  ...
  def correctSentence(self, sentence):
    ...
    for i in xrange(1, len(sentence) - 1): #ignore <s> and </s>
      ...
      # Tip: self.languageModel.score(trialSentence) gives log-probability of a sentence
        word = sentence[i]
        editProb = self.editModel.editProbabilities(word)
        #print(editProb)
        for candidate,logProb in editProb: # logProb = log(P(word))
          if candidate == word: continue # no need to evaluate the original word
          sentence[i] = candidate
          lmProb = self.languageModel.score(sentence) # lmProb = log(P(sentence))
          # log(P(x|word)P(word)) = log(P(x|word))+log(P(word))
          # a sentence could get high score even with a low P(candidate)
          # the final score of a sentence should depends on P(candidate)
          # P(sentence, candidate) = P(sentence) * P(candidate)
          # log(P(sentence, candidate)) = log(P(sentence)) + log(P(candidate))
          score = lmProb + logProb
          if score >= bestScore:
            bestScore = score
            editPos = i
            bestWord = candidate
        sentence[i] = word
    bestSentence[editPos] = bestWord
    #print(bestSentence)
    return bestSentence
